# ================================================
# BLURRED BACKGROUND ÇIKARMA — FaceForensics++
# Yüz bölgesi net, arka plan Gaussian blur ile bulanık
...
SAVE_BASE = '/content/drive/MyDrive/FaceForensics_Data/EfficientNet_Frames_BlurredBG'

# =====================
# AYARLAR
# =====================
IMG_SIZE      = 224   # EfficientNetB0 için. Xception: 299
N_FRAMES_REAL = 20    # real: 200×20 = 4000 kare
N_FRAMES_FAKE = 10    # fake: 400×10 = 4000 kare
BLUR_K        = 51    # Gaussian blur kernel boyutu
...
def isle_video(v_yolu, save_path, prefix, n_frame):
    ...
    for i, idx in enumerate(indices):
        ...
        # Tüm kareye Gaussian blur uygula
        ...
        for j in range(detections.shape[2]):
            if detections[0,0,j,2] > 0.5:
                ...
                # Yüzü orijinal (keskin) kare üzerine yapıştır
                blurred[y1:y2, x1:x2] = frame[y1:y2, x1:x2]
                break

        out = cv2.resize(blurred, (IMG_SIZE, IMG_SIZE))
...
# ANA DÖNGÜ
# =====================
print("\n🚀 Blurred Background kare çıkarma başlıyor...")
toplam_kare = 0
