# ================================================
# TEST — Xception + LSTM | Face Crop Senaryosu
# Video bazlı çoğunluk oylaması: 1, 5, 10, 50 kare
# En iyi sonuç: %89,33 doğruluk, AUC=0,9408 (10 kare)
# ================================================
...
MODEL_PATH = '/content/drive/MyDrive/FaceForensics_Data/xception_lstm_facecrop.keras'
...
SAVE_PATH  = '/content/drive/MyDrive/FaceForensics_Data/Grafikler_LSTM'
...
for n_frame in frame_sayilari:
    print(f"\n{'='*50}")
    print(f"  Kare sayısı: {n_frame} — Xception+LSTM Face Crop")
    print(f"{'='*50}")
...
plt.title('Karmaşıklık Matrisi — Xception+LSTM Face Crop (10 Kare)', fontsize=12)
plt.xlabel('Tahmin Edilen Sınıf'); plt.ylabel('Gerçek Sınıf')
plt.tight_layout()
plt.savefig(f'{SAVE_PATH}/confusion_matrix.png', dpi=150); plt.show()
...
plt.title('ROC Eğrisi — Xception+LSTM Face Crop (10 Kare)', fontsize=12)
plt.legend(loc='lower right'); plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig(f'{SAVE_PATH}/roc_curve.png', dpi=150); plt.show()

print(f"\n{'='*65}")
print(f"ÖZET — Xception+LSTM Face Crop Video Bazlı")
print(f"{'='*65}")
